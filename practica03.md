...
|---|---|
| `frontend/src/App.tsx` | Rutas `/login`, `/`, `/catalogo`, `/mi-red`, `/carrito`. Sin guardas ni roles (se agregan en esta práctica). |
| `frontend/src/context/AuthContext.tsx` | `isAuthenticated` + `userEmail`; funciones `login(email)` y `logout()`. |
| `frontend/src/context/CartContext.tsx` | Carrito **en memoria**: `addToCart`, `removeFromCart`, `totalItems`, `totalPrice`. Sin persistencia ni cantidades. Importa la interfaz `IProducto` de `components/Catalogo/interfaces/iproducto`. |
| `frontend/src/components/Login.tsx` | `fetch` a `POST /api/login`; guarda `userEmail`; navega a `/`. |
| `frontend/src/components/Catalogo/Catalogo.tsx` | `fetch` a `GET /api/productos`; grilla de tarjetas con "Añadir al Carrito". |
| `frontend/src/components/Carrito.tsx` | Lista de ítems, eliminar y total. Botón "Proceder al Pago" sin función. |
| `frontend/src/components/MiRed.tsx` | Tabla/tarjetas con datos estáticos (3 referidos). |
| `frontend/src/components/Dashboard.tsx` | 3 tarjetas con valores estáticos. |
| `frontend/src/components/Layout.tsx`, `Navbar.tsx`, `Sidebar.tsx` | Layout responsive (sidebar colapsable + móvil), carrito en Navbar, avatar con logout. |
...
**Archivo:** `backend/controllers/authController.go` (modificar)
...
**Archivo:** `frontend/src/components/Catalogo/interfaces/iproducto.ts` (actualizar)

Se mantiene el archivo de interfaz de la feature Catalogo y solo se le agregan los campos que el catálogo enriquecido necesita:

```ts
// src/components/Catalogo/interfaces/iproducto.ts
// Interfaz del producto, organizada por feature (Catalogo).
// Se mantiene este archivo y solo se agregan los campos que el detalle,
// la galería y los filtros necesitan (Tema 5).
...
**Archivo nuevo:** `frontend/src/components/Catalogo/data/productos.ts`

Los datos de ejemplo (mock) viven dentro de la feature Catalogo, usando el mismo patrón por features que `interfaces/`:

```ts
// src/components/Catalogo/data/productos.ts
// Datos de ejemplo (mock) del catálogo, organizados por feature (Catalogo).
...
> La interfaz `IProducto` se mantiene en `interfaces/iproducto.ts` (solo se actualizan sus campos). En el Paso 10 se ajustará `CartContext` para importarla desde esa ruta; el catálogo, la tienda y el detalle consumen los datos desde `data/productos.ts` y el servicio desde `services/productosService.ts` (Paso 4).
...
**Archivo nuevo:** `frontend/src/components/MiRed/data/red.ts`

```ts
// src/components/MiRed/data/red.ts
...
**Archivo nuevo:** `frontend/src/components/Catalogo/services/productosService.ts`
...
**Archivo:** `frontend/src/context/AuthContext.tsx` (reemplazar)
...
**Archivo nuevo:** `frontend/src/components/Login/services/authService.ts`

El proceso POST de autenticación se aísla en un servicio por feature, con el mismo patrón que `Catalogo/services/` (Paso 4). El componente `Login` solo consume la función:
...
**Archivo:** `frontend/src/components/Login/Login.tsx` (reemplazar — se mueve a la carpeta de la feature)

El componente ya no hace `fetch`; importa el servicio de auth con un alias para no chocar con `login` del contexto:
...
> Nota: Vite ya proxya `/api` hacia `http://localhost:3000` (`vite.config.ts`), por eso se usa la ruta relativa sin IP.
...
**Archivo:** `frontend/src/App.tsx` (reemplazar)
...
**Archivo nuevo:** `frontend/src/components/Storefront.tsx`
...
import { useAuth } from '../context/AuthContext';
import { getProductos } from '../components/Catalogo/services/productosService';
import type { IProducto } from '../components/Catalogo/interfaces/iproducto';
import { categorias } from '../components/Catalogo/data/productos';
...
**Archivo:** `frontend/src/components/Catalogo/Catalogo.tsx` (reemplazar)
...
> Nota: `addToCart` recibe ahora el `IProducto` enriquecido (con `descripcion`, `categoria`, `galeria`). En el Paso 10 se actualiza `CartContext` para importar ese tipo desde `interfaces/iproducto`.
...
**Archivo nuevo:** `frontend/src/components/DetalleProducto.tsx`
...
import { getProductoById } from '../components/Catalogo/services/productosService';
import type { IProducto } from '../components/Catalogo/interfaces/iproducto';
import { useCart } from '../context/CartContext';
...
**Archivo:** `frontend/src/context/CartContext.tsx` (reemplazar)
...
**Archivo:** `frontend/src/components/Carrito.tsx` (reemplazar)
...
**Archivo nuevo:** `frontend/src/components/Checkout.tsx`
...
**Archivo nuevo:** `frontend/src/components/Confirmacion.tsx`
...
**Archivo:** `frontend/src/components/MiRed.tsx` (reemplazar)
...
import { redInicial, contarRed, sumarVentasRed, sumarComisiones, nivelAlcanzado, comisionDeReferido, TASA_COMISION, type Referido } from './MiRed/data/red';
...
**Archivo:** `frontend/src/components/Dashboard.tsx` (reemplazar)
...
import { redInicial, contarRed, sumarVentasRed, sumarComisiones, nivelAlcanzado, comisionDeReferido } from './MiRed/data/red';
...
**Archivo:** `frontend/src/components/Sidebar.tsx` (reemplazar)
...
**Archivo:** `frontend/src/components/Navbar.tsx` (modificar)
...
- El **login** sigue consumiendo `POST /api/login` de la API (IP del aula: `localhost:3000`); ajusta la URL en `Login/services/authService.ts` si tu entorno es otro.
